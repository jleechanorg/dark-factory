...
### Python mirror (vocabulary contract sanity, r5)
```text
...
- `daemon/src/adapters.rs` — (r6) collapse nested `if` for CodeRabbit + Bugbot cap-filter; new `bugbot_pending` fake-`gh` preset; new regression test `capped_bugbot_excluded_from_ci_success_and_status`; module-local `CAP_TEST_LOCK` mutex; rework r5 CodeRabbit test to use `OnceLock::get_or_init` + in-place mutation
