...
Extracted from ``runner.skeptic_gate`` so the contract-echo bounded
subsystem (≈750 lines including its own prompt template) lives in one
canonical module. ``runner.skeptic_gate`` re-exports the public symbols
so existing callers (``runner.skeptic_gate_cli``, the skeptic test
files, and any third-party importer) keep working unchanged.

Scope
-----
- Data shapes: ``AcceptanceItem``, ``PriorFinding``, ``BeadContract``,
  ``ContractEchoItem``, ``ContractEchoReport``,
  ``ContractEchoVerdictResult``, ``PriorFindingEcho``,
  ``ContractEchoVerdict`` literal.
- Loaders: ``load_bead_contract`` (dict / path / pass-through),
  ``load_bead_contract_from_bead`` (``br show --json`` wrapper),
  and the ``_br_show_json`` subprocess shim tests monkeypatch.
- Parser + evaluator: ``parse_contract_echo``,
  ``evaluate_contract_echo``, and the ``_strip_contract_echo_block``
  helper ``parse_verdict`` calls before its strict 10-field parse.
- Prompt builder: ``build_prompt`` plus the ``_PROMPT_TEMPLATE`` and
  ``_CONTRACT_BLOCK_TEMPLATE`` formatters it formats. The contract
  branch is a strict superset of the legacy 10-field prompt; moving
  the whole ``build_prompt`` keeps the prompt shape in one module.

Internal callers (``evaluate`` in ``runner.skeptic_gate``) import from
this module so the canonical implementation lives here; the
``runner.skeptic_gate`` names are thin re-export shims.

ZFC compliance
--------------
Contract-echo is a structured-field validator (no keyword routing on
free-form prose). The parser is anchored to ``CONTRACT_ECHO:`` /
``ITEM:`` / ``PRIOR_FINDING:`` headers; unknown / malformed lines
cause the whole block to be rejected so the reviewer cannot smuggle
prose through a malformed line.
"""
...
# ---------------------------------------------------------------------------
# Contract-echo block regexes (issue #386)
# ---------------------------------------------------------------------------
#
# Anchored, case-insensitive, multi-match-detecting. The header must
# appear on its own line; ITEM / PRIOR_FINDING lines carry the strict
# per-item fields.
...
# ---------------------------------------------------------------------------
# Prompt builder (issue #386 — contract branch + legacy 10-field)
# ---------------------------------------------------------------------------
