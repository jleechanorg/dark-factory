...
Extracted from `runner/skeptic_gate` so the contract-loading, parsing,
and fail-closed evaluation of `CONTRACT_ECHO:` blocks live in their
own module. The headline invariant lives here:

    A reviewer that emits `VERDICT: PASS` without addressing every
    acceptance item (or `N-A`'ing non-required items with a reason)
    is rejected. Prior findings are enforced when the contract
    opts into `report_prior_findings=...`.

This module is self-contained — `runner.skeptic_gate` re-exports the
public symbols so legacy callers (and existing tests) keep working.
Tests that monkeypatch `_br_show_json` MUST target this module
explicitly (`monkeypatch.setattr(skeptic_contract_echo, '_br_show_json', ...)`)
rather than `skeptic_gate`, since the function lives here.

Public surface
--------------
- `AcceptanceItem`, `PriorFinding`, `BeadContract` — the durable
                                       contract shape the reviewer
                                       echoes against.
- `ContractEchoItem`, `ContractEchoReport`,
  `ContractEchoVerdictResult`, `PriorFindingEcho` — the parsed
                                       per-item verdict carriers.
- `ContractEchoVerdict`  — the verdict literal type.
- `load_bead_contract(source)` — load a contract from a dict,
                                       JSON file path, or pass-through.
- `load_bead_contract_from_bead(bead_id, br_bin=...)` — read the live
                                       bead source via `br show --json`
                                       and materialise a contract.
- `parse_contract_echo(output, contract)` — extract per-item verdicts
                                       from the `CONTRACT_ECHO:` block.
- `evaluate_contract_echo(report, contract, report_prior_findings=...)`
                                      — fail-closed check that every
                                       acceptance item is `ADDRESSED`
                                       or `N-A` (non-required items
                                       only) and every prior finding
                                       is covered when enforcement is on.
- `_strip_contract_echo_block(output)` — strip the `CONTRACT_ECHO:`
                                       block from a reviewer output so
                                       `parse_verdict` can run on the
                                       remaining 10 structured fields.
- `_br_show_json(bead_id, br_bin=...)` — subprocess wrapper around
                                       `br show <id> --json`. Exposed at
                                       module level so tests can
                                       monkeypatch it without spawning
                                       a real `br` subprocess.

ZFC compliance
--------------
This module never inspects free-form text to decide verdicts — it
parses anchored structured lines (`ITEM:`, `PRIOR_FINDING:`) and
validates the file:line cite / reason shape. Judgment belongs to the
reviewer; this code only validates the shape of what the reviewer
emitted.
"""
