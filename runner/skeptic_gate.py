...
# Contract-echo subsystem (issue #386) — extracted to its own module.
# Re-export the public surface so `from runner.skeptic_gate import …`
# keeps working for CLI, tests, and any third-party importer. The
# canonical implementation lives in `runner.skeptic_contract_echo` —
# internal callers below use those names directly so the contract-echo
# code path has one authoritative home.
from runner.skeptic_contract_echo import (  # noqa: E402,F401
    AcceptanceItem,
    BeadContract,
...
# Contract-echo subsystem (issue #386) lives in `runner.skeptic_contract_echo`.
# The block below is anchored, case-insensitive, multi-match-detecting
# regexes for the 10-field verdict (parse_verdict) and execution-evidence
# (issue #384) contracts, NOT the contract-echo block itself. The
# contract-echo types, parser, evaluator, loader, and prompt builder
# have been extracted to a sibling module; the public surface is
...
# `build_prompt` is implemented in `runner.skeptic_contract_echo` and
# re-exported at the top of this module for backward compatibility.
# Callers that need the canonical implementation should import from
