...
configure_minimax_env() {
  ...
  if [[ -z "${minimax_key//[[:space:]]/}" ]]; then
    ...
    printf 'RECOVERY: configure MINIMAX_API_KEY in a profile sourced by the dispatch script (or the launchd environment), then run: launchctl kickstart -k gui/%s/%s\n' "$(id -u)" "$PLIST_LABEL" >&2
    return 66
  fi
  local var
