...
if br --help 2>/dev/null | grep -Eq 'list.*--label'; then
...
PY
    ...
    if [ -n "$AO" ] && "$AO" session ls -p "$proj" 2>/dev/null | grep -E "pulls/${pr}\b" | grep -Eq '\[(spawning|running|active|working|pr_open)\]'; then
