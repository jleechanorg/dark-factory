...
fn run_slow_tier(deps: &TickDeps, summary: &mut TickSummary) -> Result<(), DaemonError> {
    ...
    for bead_id in &created {
        ...
        // This prevents a regression where a bead's target_repo was lost
        // if the gh pr view probe failed after computing target_repo but
        // before any overlay save.
                ...
                if let Some(ref ext_ref) = bead.external_ref {
                    if let Some((_, num_str)) = parse_external_ref(ext_ref) {
                        if let Ok(num) = num_str.parse::<u64>() {
                            ...
                            // repo (`overlay.target_repo`), not
                            // unconditionally `deps.cfg.target_repo`.
                            let probe_repo =
