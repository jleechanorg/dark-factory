...
fn run_fast_tier(deps: &TickDeps, summary: &mut TickSummary) -> Result<(), DaemonError> {
    ...
    for bead_id in &bead_ids {
        ...
        evidence.evidence_gist_status = match verifier::parse_evidence(&snapshot.body) {
            Some(parsed) => {
                ...
                if !head_matches {
                    ...
                    if !evidence_head_stale_already_recorded(deps, bead_id)? {
                        emit(
                            deps.telemetry_log,
                            bead_id,
                            overlay.attempt,
        ...
        // keeps telemetry consistent (the assessment DID happen), but
        // `continue` skips the park/reroll/escalation branches below.
...
/// coder session that the Evidence marker head SHA is stale. The bead-level
/// sentinel (`save_rejection`) keys on the same `(bead_id, attempt)` pair the
/// escalation flow uses; distinct `reviewer` string keeps the two flows
/// from colliding.
fn evidence_head_stale_already_recorded(deps: &TickDeps, bead_id: &str) -> Result<bool, DaemonError> {
    Ok(matches!(
        deps.store
...
/// fired the rejection without re-running the parser.
