...
mod parse_rest_pull_payload_tests {
    ...
    fn parse_rest_pull_payload_detects_cross_repo_pr() {
        ...
        let (
            ...
            _updated_at_epoch,
            _head_ref_name,
            _head_repo_full_name,
            _head_repo_owner_login,
            is_cross_repository,
        ) = CliScm::parse_rest_pull_payload("jleechanorg/dark-factory", &payload).unwrap();
        assert_eq!(
            head_sha.as_deref(),
...
impl Scm for CliScm {
    ...
    fn pr_snapshot(&self, pr: u64) -> Result<PrSnapshot, DaemonError> {
        ...
        for c in &checks {
            ...
            if name_lower.contains("coderabbit")
                && crate::vendor_health::is_global_vendor_capped(
    ...
    fn pr_number_for_branch(&self, repo: &str, branch: &str) -> Result<Option<u64>, DaemonError> {
        ...
        let jq_filter =
            format!(".[] | select(.headRepository.nameWithOwner == \"{repo}\") | .number");
        let out = match run_tool(
            "gh",
            &[
                "pr",
...
fn ao_spawn_command_with_mode(
    ...
    if !diagnostic {
        ...
        let verified = if spec.managed_checkout {
            // Bead jleechan-y189: managed checkouts self-heal in full
            // (stale lock, untracked caches, dirty reset, fallback branch).
            crate::target_worktree::ensure_target_worktree_self_heal(
                &spec.repo,
                checkout,
            ...
            // Bead jleechan-y189: operator checkouts self-heal the
            // crash-safe subset (stale lock, untracked caches) and refuse
            // to reset dirty state — that's the operator's call. A missing
            // branch still falls back to a fresh `origin/main` provision.
            crate::target_worktree::ensure_target_worktree_self_heal(
                &spec.repo,
                checkout,
                spec.expected_revision.as_deref(),
...
pub fn verify_ao_bridge_compatibility(
    ...
    validate_configured_vendors(installed_slice, configured_vendors).map_err(
        |error| match error {
...
mod spawn_fallback_tests {
    ...
    fn runtime_fallback_chain_preserves_passthrough_when_no_alias_matches() {
        let chain = build_runtime_fallback_chain("minimax", "claude-code->antigravity->agy");
        assert_eq!(
            chain,
            vec![
                "minimax".to_string(),
...
"#
    ...
    fn configured_checkout_source_mismatch_still_fails_closed() {
        ...
        assert!(
            !output.status.success(),
            "mismatched configured checkout unexpectedly spawned"
        );
        assert!(
        ...
        assert!(
            calls.is_empty(),
            "AO must not be invoked on a source mismatch: {calls}"
        );
    }
    ...
    fn worker_spawn_without_checkout_fails_closed_before_ao() {
        ...
        assert!(
            calls.is_empty(),
            "AO must not be invoked without a checkout"
        );
    }
    ...
    fn worker_spawn_rejects_stale_expected_revision_before_ao() {
        ...
        assert!(
            calls.is_empty(),
            "AO must not be invoked on a stale checkout"
        );
    }
    ...
    fn worker_spawn_rejects_same_origin_stale_ao_workspace_after_spawn() {
        ...
        let expected_revision = String::from_utf8(expected_revision.stdout)
            ...
            .to_string();
        let workspace =
            std::env::temp_dir().join(format!("afd_stale_ao_workspace_{}", std::process::id()));
    ...
    fn routed_spawn_uses_target_checkout_as_ao_cwd() {
        ...
        std::process::Command::new("git")
            .args([
                ...
                "add",
                "origin",
                "https://github.com/otherorg/other-repo.git",
            ])
            .current_dir(&checkout)
            .status()
            .unwrap();
        ...
        assert!(
            spawn_result.is_ok(),
            "routed spawn failed: {spawn_result:?}"
        );
        let row: serde_json::Value = calls.lines().next().unwrap().parse().unwrap();
        assert_eq!(
            row["cwd"],
            checkout.canonicalize().unwrap().to_string_lossy().as_ref()
        );
...
"#,
    ...
    fn bridge_creates_worktree_at_expected_revision_for_adopted_pr() {
        let _guard = gh_env_test_lock()
            ...
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let root =
            std::env::temp_dir().join(format!("afd_ao_bridge_adopted_pr_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        let cli = root.join("node_modules/@jleechanorg/ao-cli");
        let core = root.join("node_modules/@jleechanorg/ao-core");
        let target = root.join("managed-target");
        ...
        let pr_sha = std::process::Command::new("git")
            ...
            .output()
            .unwrap();
        let pr_sha = String::from_utf8(pr_sha.stdout).unwrap().trim().to_string();
...
"#
        ...
        assert!(
            created_wt.is_dir(),
            "worktree directory was not created: {created_wt:?}"
        );
        ...
        assert!(
            ...
            "target must own the adopted branch for this safety probe"
        );
...
impl Vcs for CliVcs {
    ...
    fn delete_branch_at_for_repo(&self, repo: &str, name: &str) -> Result<(), DaemonError> {
        ...
        let out = run_tool("gh", &["api", "--method", "DELETE", &path], 30)?;
    ...
    fn head_sha_within_for_repo(
        ...
        if sha.is_empty() || sha == "null" {
            return Err(DaemonError::Tool {
                tool: "gh".to_string(),
                rc: 0,
                stderr: format!("gh api {path} returned no sha for branch '{branch}' in {repo}"),
            });
        }
        Ok(sha.to_string())
    }
...
mod ci_dedicated_gate_ownership_tests {
    ...
    fn real_ci_and_review_bot_check_runs_stay_in_gate_1() {
        for name in [
            ...
            assert!(
                !check_owned_by_dedicated_gate(name),
                "{name} must stay in gate 1"
            );
        }
    }
}
...
"#;
    ...
    fn capped_bugbot_excluded_from_ci_success_and_status() {
        ...
        let result =
            run_pr_snapshot_with_fake_gh("not_capped_bugbot", "bugbot_pending", "fail", 45);
        let snapshot = result.expect("pr_snapshot must succeed");
        assert!(!snapshot.ci_success);
        assert!(snapshot.ci_pending);
        assert_eq!(snapshot.ci_status, "unknown");
