...
impl Vcs for CliVcs {
    ...
    /// jleechan-wuts / issue #349 (r2): routed-repo variant of
    /// [`base_head`]. `gh api repos/<repo>/git/ref/heads/<branch>` returns
    /// the SHA at the HEAD of `<branch>` in `<repo>`. Equivalent in shape
    /// to `remote_head_sha` (`repos/<repo>/commits/<branch>` returns the
    /// same SHA, but `git/ref/heads/<branch>` is the Git Data API's
    /// canonical "branch HEAD" lookup). Decoupled from the daemon's own
    ...
    /// HTTP 422 — the reroll path always passes a freshly formatted
    /// `factory/<bead>-r<attempt>` branch (incremented per attempt), so
    /// a collision in the routed repo is structurally impossible absent
    /// a stale `register_branch`/gh-state mismatch; the underlying
    /// `DaemonError::Tool` with the HTTP body as stderr surfaces that
    /// case to the operator for the same reason the old CWD-bound
    /// `git branch <name> <sha>` did.
