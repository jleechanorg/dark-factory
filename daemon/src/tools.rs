...
pub trait Scm {
    ...
    /// Repo-scoped variant of [`labeled_prs`](Scm::labeled_prs). Allows the
    /// slow-tier intake sweep to query factory-labeled PRs across all
    /// configured target repositories (`cfg.target_repo` + `cfg.repos`).
