...
mod tests {
    ...
    impl crate::tools::Scm for S {
        ...
        fn labeled_prs(
            ...
            label: &str,
            _gh_calls: &mut u32,
        ) -> Result<Vec<crate::tools::LabeledPr>, DaemonError> {
            self.calls
                .borrow_mut()
    ...
    impl StateStore for St {
        ...
        fn coalesce_owner_for(&self, _b: &str) -> Result<Option<String>, DaemonError> {
            Ok(None)
        }
    ...
    fn er_no_retrigger_when_evidence_unchanged() {
        ...
        assert!(
            ...
            "unchanged evidence must not re-spawn the reviewer"
        );
    }
    ...
    fn er_new_evidence_hash_resets_cap() {
        ...
        *llm.response.borrow_mut() = Some(Ok("/er PASS".into()));
        ...
        store
            .er_counts
            .borrow_mut()
            .insert(bead.into(), (MAX_ER_RUNNER_ATTEMPTS, Some(1)));
        ...
        store.evidence_hashes.borrow_mut().insert(
            bead.into(),
            verifier::evidence_marker_hash(
                "**Evidence**: https://gist.github.com/u/old (head deadbeef)",
            ),
        );
