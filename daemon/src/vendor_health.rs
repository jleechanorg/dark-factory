...
impl VendorHealthLedger {
    ...
    pub fn health(&self, vendor: Vendor) -> VendorHealth {
        ...
        // is one signal, not N). Build the set of distinct beads and
        // rank them by their LATEST ts_epoch so OBSERVATION_WINDOW is
        // a recency window ("the most recent N distinct capped beads"),
        // matching the doc comment. The earlier BTreeSet-based
        // implementation ranked beads lexicographically, making the
        // window dead — `recent` reduced to `min(distinct, 5)`.
        use std::collections::HashMap;
        let mut latest_ts_per_bead: HashMap<String, u64> = HashMap::new();
        for o in &obs {
            let entry = latest_ts_per_bead.entry(o.bead_id.clone()).or_insert(0);
            if o.ts_epoch > *entry {
                *entry = o.ts_epoch;
            }
        }
        let mut distinct_beads_by_recency: Vec<(String, u64)> =
            latest_ts_per_bead.into_iter().collect();
        // Sort ascending by ts_epoch so the most recent beads end up
        // at the tail; then reverse to get descending order. (We use
        // sort_by_key + reverse rather than sort_by to satisfy
        // clippy::unnecessary_sort_by and stay consistent with the
        // project's idiomatic pattern.)
        distinct_beads_by_recency.sort_by_key(|b| b.1);
        distinct_beads_by_recency.reverse();
        // Take the most recent N distinct beads (highest ts_epoch first).
        let recent = distinct_beads_by_recency
            .into_iter()
