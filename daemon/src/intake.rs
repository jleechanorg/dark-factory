...
///
/// * Sweeps every repository in `target_repositories_sweep_order(cfg)`.
/// * Fetches candidate issues labeled `factory` per repository from the SCM.
/// * Skips any issue whose `external_ref` already appears among the tracker's
///   known candidates (idempotency — no duplicate `create_bead` calls).
/// * Checks the issue author's collaborator permission tier per repository; only
///   `Permission::Write` / `Permission::Admin` pass. Lower tiers are skipped
///   (not an error) — the skip itself (issue external_ref + author_login) is
///   the audit context callers/telemetry record.
/// * For each newly-authorized issue, calls `create_bead` and collects the
///   returned bead id.
///
/// Returns `(created, outcomes)`: `created` is the ids of beads newly
/// created during this pass. `outcomes` carries exactly one `IntakeOutcome` for every
/// candidate that did NOT result in a newly-created bead (skips + errors).
///
/// No single repository or candidate's failure can prevent the rest of the batch from
/// being processed.
pub fn normalize(
    scm: &dyn Scm,
    tracker: &dyn Tracker,
    ...
    for repo in &target_repos {
        ...
        let (_, known_refs): (&[Bead], &std::collections::HashSet<String>) = match snapshot_result {
            Ok((candidates, refs)) => (candidates.as_slice(), &*refs),
            Err(()) => {
                continue;
            }
...
/// Normalize open PRs labeled `factory` into beads across all configured repositories.
pub fn normalize_labeled_prs(
    ...
    for repo in &target_repos {
        ...
        let prs_result = scm.labeled_prs_for_repo(repo, FACTORY_LABEL, &mut legacy_gh_calls);
        let prs = match prs_result {
            Ok(p) => p,
            Err(e) => {
                eprintln!(
                    "auto-factory daemon: WARNING PR intake failed for repository {repo}: {e}; continuing with remaining repositories"
                );
                outcomes.push(IntakeOutcome {
                    external_ref: repo.clone(),
                    verdict: IntakeVerdict::Errored {
        ...
        let snapshot_result = tracker_snapshot_state.get_or_insert_with(|| {
            match (tracker.fetch_candidates(), tracker.fetch_all_external_refs()) {
                ...
                (Err(e), _) | (_, Err(e)) => {
                    eprintln!(
                        "auto-factory daemon: WARNING PR intake tracker snapshot fetch failed \
                         (first needed by repository {repo}): {e}; degrading PR intake for the \
                         remainder of this sweep"
                    );
                    Err(())
