...
mod tests {
    ...
    fn spawn_branch_mismatch_clears_stale_pr_number_so_daemon_can_rediscover_current_pr() {
        ...
        // Persist via the in-memory map directly to mirror the live "durable
        // overlay already on disk" precondition (FakeStateStore's `save`
        // API is the trait seam but we need to seed pre-dispatch state).
        store.overlays.borrow_mut().insert("bead-0".into(), preloaded);
        ...
        assert!(
            ...
            "stale pr_number (the closed #316) must be cleared on \
             spawn_branch_mismatch so the next dispatch can re-resolve the \
             current correct PR; instead the overlay still carried \
             pr_number={:?}",
            overlay.pr_number,
        );
        assert!(
