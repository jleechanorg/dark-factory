...
# Use --manifest-path so cargo runs from REPO_ROOT (not daemon/) — the
# detector derives file paths via `git diff` and they are repo-relative;
# running cargo from inside daemon/ would double-prefix them.
cargo build --quiet --bin vacuous_red_green --manifest-path daemon/Cargo.toml

# Run the detector; capture the exit code.
RC=0
cargo run --quiet --bin vacuous_red_green --manifest-path daemon/Cargo.toml -- "${INVOKE[@]}" || RC=$?
