...
valid_branch()  {
  # Default: factory/<bead_id>-r<n> for new-work beads
  [[ "$1" =~ ^factory/[A-Za-z0-9._-]+-r[0-9]+$ ]] && return 0
  # Allow drive-existing-pr: existing-PR branch names (e.g., fix/quota-banner-modal-cta-7945)
  # Triggered when bead body has `existing_branch:` field — coder SKILL.md documents the convention.
  # Excludes factory/* and refs/heads/* prefixes to keep the dispatch surface unambiguous.
  if [[ "$1" =~ ^[A-Za-z0-9._/-]+$ ]] && [[ ! "$1" =~ ^factory/ ]] && [[ ! "$1" =~ ^refs/ ]] && [[ "$1" != "HEAD" ]]; then
    return 0
  fi
  die "branch must match factory/<bead_id>-r<n> OR an existing-PR branch name: $1"
}
