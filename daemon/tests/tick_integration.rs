...
// jleechan-80k9 / bead dark-factory-80k9: a deferred reroll suppresses
// duplicate assessment of the OLD PR, but that suppression must NOT leak into
// a later, durably discovered PR for the fresh attempt. The DISPATCHED ->
// ATTESTED promotion is that durable boundary. When the fresh attempt's
// `branch_to_pr` binding fires and the fast tier promotes the overlay to
// ATTESTED, the previous attempt's `reroll_deferral_count` must be reset to
// 0 so the next fast tier pass actually gate-assesses the fresh PR (instead
// of hitting the jleechan-msmq guard and emitting
// VERIFIER_SKIPPED_REROLL_IN_PROGRESS against an already-superseded bead).
#[test]
fn tick_resets_reroll_deferral_when_fresh_attempt_pr_opens() {
    ...
    *llm.response.borrow_mut() = Some(Ok("pass".into()));
    ...
    // A prior reroll attempt deferred on the OLD PR — the
    // `reroll_deferral_count` is now 1, which would normally trigger the
    // jleechan-msmq guard.
    store.incr_reroll_deferral("fresh-attempt").unwrap();
    assert_eq!(store.reroll_deferral_count("fresh-attempt").unwrap(), 1);

    // Model a freshly established retry: it has a branch-to-PR binding and
    // is promoted from DISPATCHED only after that binding has been observed.
