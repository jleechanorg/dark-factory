...
fn transient_spawn_failures_below_cap_stay_retriable_and_do_not_park() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    // Bead jleechan-yvlq / G12 retry-backoff bleed: each transient failure
    // now stamps a per-bead `next_retry_at` so the slow tier skips the bead
    // until the backoff window elapses. The test runs 3 ticks in immediate
    // succession (real wall-clock doesn't advance), so we manually clear
    // `next_retry_at` between ticks to simulate the 4-minute tick cadence
    // and exercise the failure-count mechanics. The cross-tick skip path
    // itself is covered by `dispatch::tests::g12_*`.
    for tick_index in 0..3u64 {
        // Simulate per-bead backoff window elapsing so this tick
        // re-attempts the bead.
        if tick_index > 0 {
...
fn transient_spawn_retry_cap_exceeded_parks_human_held_with_escalation() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    // Bead jleechan-yvlq / G12 retry-backoff bleed: each transient failure
    // now stamps a per-bead `next_retry_at` so the slow tier skips the bead
    // until the backoff elapses. The test runs ticks in immediate
    // succession (real wall-clock doesn't advance), so we clear
    // `next_retry_at` BEFORE each tick to simulate the 4-minute tick
    // cadence outpacing the per-bead backoff floor.
    for tick_index in 1..=14u64 {
        // Simulate per-bead backoff window elapsing so this tick
        // re-attempts the bead.
        let mut next_overlay = store.load(BEAD_ID).unwrap().unwrap();
        next_overlay.next_retry_at = None;
        store.save(&next_overlay).unwrap();
        let summary = run_tick(&deps, tick_index, 0)
            .unwrap_or_else(|e| panic!("tick {tick_index} should succeed: {e:?}"));
        assert_eq!(
            summary.beads_dispatched, 0,
    ...
    // Bead jleechan-yvlq / G12 retry-backoff bleed: clear `next_retry_at`
    // before the cap tick so the dispatch path actually runs.
    let mut next_overlay = store.load(BEAD_ID).unwrap().unwrap();
    next_overlay.next_retry_at = None;
    store.save(&next_overlay).unwrap();
...
fn spawn_failure_count_resets_after_a_successful_dispatch() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    for tick_index in 0..2u64 {
        // Bead jleechan-yvlq / G12 retry-backoff bleed: simulate the
        // per-bead backoff window elapsing BETWEEN ticks so the next tick
        // re-attempts the bead (production waits 4 min between ticks;
        // the backoff floor is 30s).
        if tick_index > 0 {
            let mut overlay = store.load(BEAD_ID).unwrap().unwrap();
            overlay.next_retry_at = None;
            store.save(&overlay).unwrap();
    ...
    // Bead jleechan-yvlq / G12 retry-backoff bleed: clear the per-bead
    // backoff stamp so the recovery tick can actually attempt dispatch.
    let mut next_overlay = store.load(BEAD_ID).unwrap().unwrap();
    next_overlay.next_retry_at = None;
    store.save(&next_overlay).unwrap();
...
fn deferred_spawn_backpressure_never_increments_counter_or_parks_across_repeated_cycles() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    for tick_index in 0..N {
        // Bead jleechan-yvlq / G12 retry-backoff bleed: Deferred spawn
        // failures do NOT stamp `next_retry_at` (Deferred is requeued, not
        // parked, and the per-bead backoff is isolated to genuine transient
        // errors). Belt-and-suspenders: clear any stray `next_retry_at`
        // between ticks so the test exercises the failure-count mechanics
        // deterministically.
        if tick_index > 0 {
            let mut overlay = store.load(BEAD_ID).unwrap().unwrap();
            overlay.next_retry_at = None;
...
fn mixed_batch_deferred_backpressure_and_genuine_transient_failures_are_independent() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    // Bead jleechan-yvlq / G12 retry-backoff bleed: each transient failure
    // on bead B now stamps a per-bead `next_retry_at` so the slow tier skips
    // bead B until the backoff elapses. The test runs ticks in immediate
    // succession (real wall-clock doesn't advance), so we clear
    // `next_retry_at` before each tick to simulate the 4-minute tick
    // cadence outpacing the per-bead backoff floor.
    for tick_index in 0..=14u64 {
        if tick_index > 0 {
            // Simulate per-bead backoff window elapsing so this tick
            // re-attempts bead B.
            let mut next_overlay = store.load(BEAD_B).unwrap().unwrap();
            next_overlay.next_retry_at = None;
            store.save(&next_overlay).unwrap();
        }
        let summary = run_tick(&deps, tick_index, 0)
