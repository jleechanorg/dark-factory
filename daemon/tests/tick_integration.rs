...
fn transient_spawn_failures_below_cap_stay_retriable_and_do_not_park() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    // a transient spawn failure now sets `next_attempt_at = now + per_bead_backoff_secs(1)``
    // (5 s with the G12 bounded backoff). The second tick in the same
    // wall-clock window will skip this bead because `now_epoch < next_attempt_at`.
    // To exercise the cap, manipulate the per-bead backoff directly so the
    // bead is dispatchable on each tick.
    for tick_index in 0..3u64 {
        if tick_index > 0 {
            let mut overlay = store.load(BEAD_ID).unwrap().unwrap();
            overlay.next_attempt_at = None;
            overlay.spawn_failure_count = tick_index as u32;
            store.save(&overlay).unwrap();
        }
        let summary = run_tick(&deps, tick_index, 0)
            .unwrap_or_else(|e| panic!("tick {tick_index} should succeed: {e:?}"));
...
fn transient_spawn_retry_cap_exceeded_parks_human_held_with_escalation() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    for tick_index in 1..=14u64 {
        // Bead jleechan-w4q3 (G12): clear the per-bead backoff gate so the
        // tick can re-attempt spawn. The wall-clock in this test does not
        // advance between ticks, so the gate would otherwise filter the
        // bead out of the ready list.
        let mut overlay = store.load(BEAD_ID).unwrap().unwrap();
        overlay.next_attempt_at = None;
        store.save(&overlay).unwrap();
        let summary = run_tick(&deps, tick_index, 0)
...
fn spawn_failure_count_resets_after_a_successful_dispatch() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    for tick_index in 0..2u64 {
        if tick_index > 0 {
            // Bead jleechan-w4q3 (G12): the prior tick's transient spawn
            // failure set `next_attempt_at = now + per_bead_backoff_secs(1)`
            // (5 s). Clear it so the next tick can re-attempt the spawn.
            let mut overlay = store.load(BEAD_ID).unwrap().unwrap();
            overlay.next_attempt_at = None;
            overlay.spawn_failure_count = tick_index as u32;
            store.save(&overlay).unwrap();
...
fn mixed_batch_deferred_backpressure_and_genuine_transient_failures_are_independent() {
    ...
    *llm.response.borrow_mut() = Some(Ok(
    ...
    for tick_index in 0..=14u64 {
        // Bead jleechan-w4q3 (G12): the prior tick's transient spawn failure
        // set `next_attempt_at = now + per_bead_backoff_secs(N)` on bead B.
        // Clear it so this tick can re-attempt the spawn. Bead A is
        // `Deferred` (backpressure), which never touches the per-bead backoff,
        // so it does not need clearing. The wall-clock in this test does not
        // advance between ticks, so the gate would otherwise filter bead B
        // out of the ready list.
        if tick_index > 0 {
            if let Some(mut overlay_b) = store.load(BEAD_B).unwrap() {
                overlay_b.next_attempt_at = None;
                store.save(&overlay_b).unwrap();
            }
        }
        let summary = run_tick(&deps, tick_index, 0)
            .unwrap_or_else(|e| panic!("tick {tick_index} should succeed: {e:?}"));
