...
fn genuine_red_green_passes_check() {
    let mut proj = build_mini_project(ProjectKind::GenuineRedGreen);
    let _ = commit_current_tree(&mut proj, "feat: real test");
...
fn vacuous_test_is_flagged() {
    let mut proj = build_mini_project(ProjectKind::VacuousAlwaysGreen);
    let _ = commit_current_tree(&mut proj, "feat: vacuous test");
    let changed = classify_changed_files(&proj);
