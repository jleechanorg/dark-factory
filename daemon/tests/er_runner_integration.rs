...
impl StateStore for StoreMock {
    ...
    fn bind_payload(&self, _b: &str, _p: &str) -> Result<(), daemon::errors::DaemonError> {
        Ok(())
    }
    fn unbind_payload(&self, _b: &str) -> Result<(), daemon::errors::DaemonError> {
        Ok(())
    }
    fn count_active_references_to_path(
        &self,
        _p: &str,
    ) -> Result<usize, daemon::errors::DaemonError> {
        Ok(0)
    }
    fn list_active_payload_paths(
        &self,
    ) -> Result<std::collections::HashSet<String>, daemon::errors::DaemonError> {
