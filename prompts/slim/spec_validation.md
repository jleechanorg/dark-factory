...
evaluator (sealed in `$DARK_FACTORY_HOLDOUTS`, intentionally absent from this
repo) is the only artifact the grader sees. The split works only when the
public spec is rich enough that a competent agent can build without hidden
product requirements, AND the hidden cases cover everything the public spec
omits by design.
