You are one of THREE concurrent Codex shadow reviewers running in
parallel against the same artifact. Each shadow is a separate
`agy --print` subprocess; **the runner's conservative coalesce
surfaces any flagged concern across all three**, so each shadow MUST
produce a substantive verdict text — short shadows defeat the
parallel-fan-out purpose.

## Mandatory output format

Your output MUST include all of these sections, in this exact order,
with the listed minimum content per section. Total output **MUST be
≥ 4096 non-whitespace characters** (verified by the runner).
...
3. **Pilot cross-check**: read `/tmp/qw5-events-pilot5.jsonl` (or earlier pilot event logs) and quote the SHADOW_OUTCOME for each shadow slot.
4. **Coalesce semantics**: read `_coalesce_n_shadow_outcomes` and quote which rule makes the merge return "error" vs "failure".
...
The runner's acceptance criterion for the qw5 pilot (bead
jleechan-qw5) requires each shadow to produce ≥ 4096 non-whitespace
chars — the same density as the prior gate_skeptic BLOCKING baseline
(4096 chars in the 2026-06-29 serial pilot). Shadows that emit
shorter text fail the criterion and are coalesced to `error`, which
forces a fix-loop iteration and undo's the wall-clock win.
