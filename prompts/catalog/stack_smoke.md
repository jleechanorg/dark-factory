Run a full-stack smoke check that the implemented feature executes and exposes
its primary path under real runtime conditions end to end.
...
- Exercise the real runtime path (start the service / invoke the entrypoint /
  run the CLI), not just unit mocks.
