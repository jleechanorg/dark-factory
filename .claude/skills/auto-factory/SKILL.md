...
CommentsResolved, EvidenceFloor, Skeptic, VacuousRedGreen`). The `code_standards` and `zfc` checks are
...
shared with the original 8 gates (7 canonical + VacuousRedGreen); **no parallel implementation**. `unknown`
