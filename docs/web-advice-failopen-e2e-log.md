...
| **B** | **Anchor-comment pattern should default to `//` not `#`** | **FIX** | **Shipped:** Created `docs/code-standards.md` explicitly defining language-specific comment syntax conventions for anchor comments (Rust `.rs` uses `//`, Python `.py` uses `#`, Shell `.sh` uses `#`, YAML `.yml` uses `#`, SQL `.sql` uses `--`, JS/TS uses `//`). Added pre-push / staged syntax validator `scripts/check_anchor_comment_syntax.py` and test suite `tests/test_anchor_syntax.py`. |
