...
| PR review only (no implementation) | `pipelines/slim/review_pr.dot` | cold review → evidence gate; no holdout; agy/claude |
| Parallel fan-out/fan-in demo | `pipelines/parallel_demo.dot` | three concurrent branches + join (2-of-3 quorum); echo backend demo |
...
`gates`, `hello`, `pr_gates`, `minimal_pr`, `minimal_feature`, `minimal_research`, `bug_fix`, `review_slim`, `review_full`, `spec_gen`, `review_pr`, `parallel_demo`
