...
def test_libpython_shim_skipped_when_python_already_links_libpython():
    """The shim install MUST be skipped when the toolchain python already
    links libpython3.13 via DT_NEEDED.

    Background: on actions/setup-python Linux x64/arm64 runners, the
    toolchain python binary at `$TOOLCHAIN_ROOT/bin/python3.real` has
    `libpython3.13.so.1` in its DT_NEEDED entries (verifiable with
    `ldd $TOOLCHAIN_ROOT/bin/python3.real | grep libpython`). The
    versioned-symlink fix at step (4) plus the LD_LIBRARY_PATH export
    at step (3) are sufficient in that case — the dynamic linker
    resolves libpython via the standard DT_NEEDED path.

    The PR #287 step 4b "shim install" path installs a `python3` bash
    script that `exec env LD_PRELOAD=$SO_PATH $SHIM_REAL "$@"`. On
    Linux x64/arm64 self-hosted runners that double-loads libpython3.13.so.1.0
    into the python process (once via DT_NEEDED, once via dlopen on the
    LD_PRELOAD path). The CPython runtime hits a GIL/TLS conflict during
    `Py_Initialize` / `Py_Finalize` and `python -c 'import sys;
    sys.exit(0)'` wedges forever.

    Verified on PR #303 ez-runner-c-4 test-job run 29621097672 2026-07-17:
    the `timeout 60` self-test wrapped the hung process and surfaced as
    rc=124 after 60 seconds.

    The fix is a structural guard: the shim install block must check
    whether the toolchain python already has libpython in DT_NEEDED
    (typically via `ldd ... | grep libpython`) and skip the LD_PRELOAD
    shim path when it does. Without this guard, the self-test timeout
    fires on every Linux x64/arm64 self-hosted runner and the `test`
    job fails.

    The assertion accepts either of these patterns as evidence the
    guard exists:
    - `ldd "$SHIM_REAL"` / `ldd ... python3.real` referencing the
      toolchain binary
    - A reference to `DT_NEEDED` in a comment
    - A reference to "double-load" or "double load" in a comment
    """
    run = _libpython_fix_step_run(WORKFLOW.read_text())
    has_dt_needed_check = (
        'ldd "$SHIM_REAL"' in run
        or "ldd \"$SHIM_REAL\"" in run
        or "ldd $SHIM_REAL" in run
        or "DT_NEEDED" in run
        or "double-load" in run
        or "double load" in run
    )
    assert has_dt_needed_check, (
        "libpython fix step must skip the LD_PRELOAD shim install when "
        "the toolchain python already links libpython3.13 via DT_NEEDED. "
        "Without this guard, the self-test wedges on Linux x64/arm64 "
        "self-hosted runners (PR #303 ez-runner-c-4 test-job run "
        "29621097672 2026-07-17, rc=124 from the 60s timeout). "
        "Look for a structural check that uses `ldd $SHIM_REAL` "
        "(or similar) to detect an existing libpython link, OR a "
        "comment explicitly explaining why the shim is skipped when "
        "DT_NEEDED already resolves libpython. The current step has "
        "neither the structural check nor the explanation."
    )
