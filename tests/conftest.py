...
def run_conformance(*args: str, timeout: int = 600, env: dict | None = None) -> "subprocess.CompletedProcess[str]":
    ...
    For the `run` subcommand, inject an OS-tempdir scratch as `--workdir`
    so the subprocess runner's branch_* mkdtemp dirs land in the OS tempdir
    instead of accumulating at the repo root (the branch_* disk leak).
    """
    full_args = args
    ...
    if sub == "run":
        ...
        # Mirror the impl tree so the runner's `workdir/impl` symlink works
        # without copying every impl file for every test invocation.
...
# Registry of module-level SCRATCH workdirs created by tests that default
# `Context(workdir=...)` to a tempfile.mkdtemp. Tests register their SCRATCH
# path via `register_scratch_dir(SCRATCH)` at import time (after the mkdtemp
# call); the pytest_sessionfinish hook below removes them on session teardown
# so the OS tempdir doesn't accumulate branch_* sibling dirs across many
# pytest runs.
