...
def test_default_two_node_does_not_select_controller_snapshot_path() -> None:
    """The fresh-terminal default has no controller base or snapshot contract."""
    from runner.parser import parse

    graph = parse(Path(__file__).parents[1] / "pipelines/slim/two_node.dot")
    reviewer = graph.nodes["cold_reviewer"]
    assert reviewer.attrs["type"] == "codergen"
