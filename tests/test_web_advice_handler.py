...
class TestSuccessfulRun:
    ...
    def test_renders_prompt_template_and_enforces_timeout(self, tmp_path, monkeypatch):
        """Prompt reference (@prompts/web_advice.txt) is rendered with placeholders and timeout is passed."""
        monkeypatch.setattr(
            "runner.handler_web_advice._probe_aside_mcp", lambda c: True
        )
        monkeypatch.setattr(
        ...
        def _fake_subprocess(transport, prompt, *, timeout=None):
            ...
            return {
                ...
                "verdict": "approve",
                "summary": "OK",
                "share_urls": ["https://g.co/gemini/share/test123"],
                "returncode": 0,
                "stderr": "",
                "transport": transport,
            }
        ...
        ctx = _make_ctx(tmp_path)
        ctx.state["feature"] = "test-feature"
        node = Node(
            name="web_advice",
            attrs={
                "type": "web_advice",
        ...
        # Verify rendered prompt was passed, not the literal '@prompts/web_advice.txt'
        assert call["prompt"] != "@prompts/web_advice.txt"
        assert "You are the **/web-advice** advisory reviewer" in call["prompt"]
        assert "https://github.com/jleechanorg/dark-factory/pull/655" in call["prompt"]
