...
# Extract the pure-python block (lines 41-123): line 40 is the bash
# `printf | python3 -c '` wrapper, line 124 is the trailing `'\'' "$live_head"`.
...
# 2b. Canonical 8-gate set all-pass (jleechan-ni1k #2: vacuous_red_green
# is now part of the canonical set) + fresh head -> no-fail.
emit_assessment "$FULL8" "$LIVE_HEAD" ""
last="$(tail -1 "$LOG")"
set +e
out="$(run_predicate "$last")"; rc=$?
set -e
assert "canonical 8-gate all-pass + fresh head -> exit 0" "0" "$rc"
