...
assert_grep "unit has watchdog sized for long LLM ticks" '^WatchdogSec=7200s$' "$UNIT"
assert_grep "unit sets KillSignal to SIGINT" '^KillSignal=SIGINT$' "$UNIT"
assert_grep "unit sets KillMode to process to preserve active sessions" '^KillMode=process$' "$UNIT"
