...
def test_slim_review_prompt_contains_no_domain_bias_terms(banned_term: str) -> None:
    ...
    must remain domain-agnostic. Any reappearance of a D&D / world-architect
    term (level-up, wizard, Fighter, campaign class, ...) or the
    streaming-app-specific evidence filename (streaming_evidence) means a
    refactor accidentally re-leaked the legacy benchmark-specific phrasing
    into the generic reviewer. This pins the 2026-06-27 audit Lane A
    acceptance criteria for bead jleechan-9bi.
    """
    text = _read_prompt("prompts/slim/review.md")
...
def test_slim_review_prompt_contains_no_vendor_specific_filenames(
    ...
    assert banned_filename not in text, (
        ...
        f"placeholder (<primary-evidence.json> or <run-trace.jsonl>)."
    )
